...
export const quickSort = async (arr: Ref<number[]>, state: Ref<State[]>, start: number, end: number) => {
    if (start >= end) return;
    let pivot = Math.floor((start + end) / 2);
    state.value[pivot] = State.pivot;
    let index = await partition(arr, state, start, end, pivot);
    ...
    await quickSort(arr, state, start, index - 1);
    for (let i = start; i < index; i++) {
        state.value[i] = State.sorting;
    }
    await quickSort(arr, state, index, end);
    for (let i = index; i <= end; i++) {
        state.value[i] = State.sorting;
    }
}

const partition = async (arr: Ref<number[]>, state: Ref<State[]>, start: number, end: number, pivot: number) => {
    var left = start;
    var right = end;
